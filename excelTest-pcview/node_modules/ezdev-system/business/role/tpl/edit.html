<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>
        角色编辑
        <small>编辑角色信息</small>
    </h1>
</section>

<!-- Main content -->
<section class="content">


    <div class="row">
        <div class="col-xs-12" style="min-width:520px">
            <div class="box ">

                <!--   <div class="box box-warning  ">-->
                <div class="box-header with-border">
                    <h3 class="box-title">编辑</h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <form id="submitForm" role="form">

                        <!-- hidden input -->
                        <input type="hidden" name="versionNum" value="{{data.versionNum}}" class="form-control">
                        <!-- text input -->
                        <input type="hidden" name="id" class="form-control" value="{{data.id}}" readonly="readonly"/>
                        <div class="form-group col-sm-6">
                            <label><i class="fa"></i><span
                                    style="color: red;font-weight: bolder">*</span>&nbsp;角色名称</label>
                            <input type="text" name="name" class="form-control" value="{{data.name}}" placeholder="请输入角色名称"
                                   datatype="s1-10" errormsg="请填写1到10位中文、英文、数字和下划线！" onchange="this.value=this.value.replace(/(^\s*)|(\s*$)/g,'')" />
                            <span class="help-block"></span>
                        </div>
                        <div style="margin:0 auto;" class="form-group col-sm-6">
                            <label><i class="fa"></i><span
                                    style="color: red;font-weight: bolder">*</span>&nbsp;角色编码</label>
                            <input type="text" name="code" class="form-control" value="{{data.code}}" readonly="readonly"
                                   placeholder="请输入角色编码" datatype="/^[0-9A-Za-z_]{1,20}$/"
                                   onchange="this.value=this.value.replace(/(^\s*)|(\s*$)/g,'')"
                                   errormsg="请填写1到20位英文、数字和下划线！"  />
                            <span class="help-block"></span>
                        </div>
                        <div class="form-group col-sm-12"><span><p></p></span></div>
                        <div style="margin:0 auto;" class="form-group col-sm-6">
                            <label><i class="fa"></i>&nbsp;角色描述</label>
                            <textarea rows="5" name="remark" class="form-control" ignore="ignore"
                                      placeholder="请输入角色描述" datatype="*0-200"
                                      onchange="this.value=this.value.replace(/(^\s*)|(\s*$)/g,'')" >{{data.remark}}</textarea>
                            <span class="help-block"></span>
                        </div>

                        <div class="box-footer col-sm-12">
                            <button e-event="href:/{{path}}/submit?type=post&url=/system/core/role/save&contentType=application/x-www-form-urlencoded&goUrl=/{{path}}/index"
                                    class="btn btn-success" id="save_btn" type="button" title="保存信息"
                                    >
                                <i class="fa fa-arrow-up"></i>&nbsp;保存信息
                            </button>
                            <button e-event="href:/{{path}}/goBack?goUrl=/{{path}}/index" class="btn btn-primary " id="back_btn"
                                    type="button" title="返回">
                                <i class="fa fa-undo"></i>&nbsp;返回
                            </button>
                        </div>

                    </form>
                </div>
                <!-- /.box-body -->


            </div>
            <!-- /.box -->
        </div>
        <!-- /.col -->
    </div>


    <!-- Your Page Content Here -->

</section>
<!-- /.content -->

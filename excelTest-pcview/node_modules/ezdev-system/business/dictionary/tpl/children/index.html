<!-- Content Header (Page header) -->
<section class="content-header">
    <h1>
        字典管理
        <small>字典管理</small>
    </h1>
    <!--<ol class="breadcrumb">
        <li><a href="#"><i class="fa fa-dashboard"></i> 字典管理</a></li>
        <li class="active">字典管理</li>
    </ol>-->
</section>

<!-- Main content -->
<section class="content">

    <div class="row">

        <form id="searchForm">
            <div class="col-xs-12">
                <div class="box">
                    <div class="box-header">
                        <h3 class="box-title"></h3>
                    </div>
                    <!-- /.box-header -->
                    <div class="box-body">
                        <div class="Tables_wrapper form-inline dt-bootstrap">
                        	<input id="parentId" name="search_EQ_parentDic.id" type="hidden" value="{{if parentid!=null}}{{parentid}}{{else}}0{{/if}}">
                            <div class="row">
                            	<div class="col-sm-5" style=" min-width:350px;">
                                    <label>字典名称：
                                        <div class="input-group date">
                                            <input type="text" name="search_LIKE_name" class="form-control"
                                                   style="min-width:120px;">
                                        </div>
                                    </label>
                                </div>
                                <!--<div class="col-sm-2" style=" min-width:350px;">
                                    <label>字典编码：
                                        <div class="input-group date">
                                            <input type="text" name="search_LIKE_code" class="form-control"
                                                   style="min-width:120px;">
                                        </div>
                                    </label>
                                </div>-->

								
            					<input type="hidden" name="sortType" id="sort" value="sortNo">

                                <div class="col-sm-6">
                                    <div class="inline pull-right" style="margin-right: 10px;">
                                        <button e-event="href:/{{path}}/goBack" class="btn btn-primary" {{if parentid==null}} disabled="disabled"{{/if}} id="back_btn" type="button" title="返回">
                                        <i class="fa fa-undo"></i>&nbsp;返回父字典
                                        </button>
                                    </div>

                                    <div class="inline pull-right" style="margin-right: 10px;">
                                        <button id="add" type="button" class="btn btn-block btn-success"
                                                e-permission="core_dictionary_create"
                                                e-router="href:/{{path}}/addDic?tpl=add"><!--style="min-width:100px;"-->
                                            <li class="fa fa-plus"></li>
                                            &nbsp;新建
                                        </button>
                                    </div>
                                    <div class="inline pull-right" style="margin-right: 10px;">
                                        <button type="button" title="查询"
                                                e-event="href:/{{path}}/search?url=/system/core/dictionary/list&tpl=children/list,auto:true"
                                                class="btn btn-block btn-primary"><!--style="min-width:100px;"-->
                                            <li class="fa fa-search-plus"></li>
                                            &nbsp;查询
                                        </button>
                                    </div>


                                </div>
                            </div>
                            <div id="list" class="row">

                            </div>
                        </div>
                    </div>
                    <!-- /.box-body -->
                </div>
                <!-- /.box -->
            </div>
        </form>
        <!-- /.col -->
    </div>
    <!-- /.row -->

</section>
<!-- /.content -->

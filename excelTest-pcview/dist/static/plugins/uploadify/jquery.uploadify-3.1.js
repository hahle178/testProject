...
(function($) {
			...
			return this
					.each(function() {
						...
						if (flashInstalled) {
							...
							//add lhw 20160118
							if( settings.customUdr ){
								$button = $('<div />', {
									'id' : settings.id + '-button',
									//'href':settings.customUdrHref,
									'tpl':"uploader/index",
									'href':"uploader/index",  
									'operation' : 'showDialog',
									'dialogwidth':'500px',
									'customPara':settings.customPara,
									'silence' : 'true',
									'dialogtype':"create",
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			//MODIFY LHW 20151125 修改报错信息 START
			//this.queueData.errorMsg = 'Some files were not added to the queue:';
			this.queueData.errorMsg = '文件未加入上传列表，请确认是否有如下问题： 格式(后缀)不正确 ';
			//MODIFY LHW 20151125 END
		...
		onSelect : function(file) {
			...
			for ( var n in this.queueData.files) {
				...
				if (queuedFile.uploaded != true && queuedFile.name == file.name) {
					...
					//del lhw 20160118 同名文件在上传队列中时，直接让用户等待前一个上传完毕。
					//					var replaceQueueItem = confirm('同名文件： "'
					//							+ file.name
					//							+ '" 正在上传中，请等待其上传完毕后再上传.');
					//										
					//					if (!replaceQueueItem) {
		...
		onSelectError : function(file, errorCode, errorMsg) {
			...
			if ($.inArray('onSelectError', settings.overrideEvents) < 0) {
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					...
					this.queueData.errorMsg += ' / 大小超出设定：('+ settings.fileSizeLimit + ') ';
					break;
				...
				case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
					...
					//		+ settings.fileTypeDesc + ').';
					this.queueData.errorMsg += ' / 格式(后缀)不正确 ';
					break;
				}
			}
